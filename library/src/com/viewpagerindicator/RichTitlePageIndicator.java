...
/**
 * A RichTitlePageIndicator is to provide a android play like title page indicator
 * features include:
 * - custom cliping and fading of prev/next item
 * - add icon to title, and option to show it only in currenct item
 * Implementation:
 * reusing the touch event in TitlePageIndicator and reimplement the onDraw function
 */
public class RichTitlePageIndicator extends View implements PageIndicator {

    private static final int SIDE_ITEM_MAX_WIDTH = 150;
    ...
    public RichTitlePageIndicator(Context context) {
        this(context, null);
    }

    public RichTitlePageIndicator(Context context, AttributeSet attrs) {
    ...
    private void clipViewOnTheRight(Rect curViewBound, float curViewWidth, int right) {
        if (curViewWidth < SIDE_ITEM_MAX_WIDTH) {
            curViewBound.right = (int) (right - mClipPadding);
            curViewBound.left = (int) (curViewBound.right - curViewWidth);
        } else {
            curViewBound.left = (int) (right - mClipPadding - SIDE_ITEM_MAX_WIDTH);
    ...
    private void clipViewOnTheLeft(Rect curViewBound, float curViewWidth, int left) {
        if (curViewWidth < SIDE_ITEM_MAX_WIDTH) {
            ...
            curViewBound.right = (int) (left + mClipPadding + SIDE_ITEM_MAX_WIDTH);
            curViewBound.left = (int) (curViewBound.right - curViewWidth);
        }
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (heightSpecMode == MeasureSpec.EXACTLY) {
            ...
            for (int i=0;i<boundsSize;i++){
                boundHeight = (int)Math.max(height, bounds.get(i).bottom);
            }
            height = boundHeight + mFooterLineHeight + mFooterPadding + mTopPadding;
    ...
    private String getTitle(int pos){
        return ((TitleProvider)mPagerAdapter).getTitle(pos);
    }
    private Bitmap getIcon(int pos){
        return ((TitleProvider)mPagerAdapter).getIcon(pos);
    }
}
